...
for k=0:(N-1) 
    for n=0:(N-1) 
        X_k(k+1)=X_k(k+1)+((encrypted_msg_arr(n+1))*exp(-i*2*pi*k*n/N)); 
    end 
end 
...
for n=0:(N-1) 
    for k=0:(N-1) 
        IX_n(n+1)=IX_n(n+1)+((1/N)*((X_k(k+1)*exp(i*2*pi*k*n/N)))); 
    end 
end 
...
subplot(2,2,1); 
stem(range_msg,msg_arr);xlabel('n');ylabel('Amplitude');title('Message Signal'); 
subplot(2,2,2); 
stem(c,encrypted_msg_arr);xlabel('n');ylabel('Amplitude');title('Encrypted 
signal'); 
subplot(2,2,3); 
stem(c,abs(X_k));xlabel('k');ylabel('Magnitude of X_k');title('Magnitude 
characteristics of encrypted signal'); 
subplot(2,2,4); 
stem(c,(angle(X_k)*180/pi));xlabel('k');ylabel('Phase of X_k');title('Phase 
characteristics of encrypted signal'); 
